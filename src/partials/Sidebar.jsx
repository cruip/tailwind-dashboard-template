...
function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  const sideBarLink = [
    ...
    },
    {
      ...
      to: dashboardRoutes.PAYMENT_METHODS,
      iconName: "pricing",
    },
    {
      label: "Customer's route req",
      to: dashboardRoutes.CUSTOMER_ROUTE_REQ,
      iconName: "users",
    },
  ];
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span
                            className={`ml-3 mr-5 text-sm font-medium duration-200 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100`}
                          >
                            {label}
                          </span>
                          {/* {subMenu && <div>
                            <button onClick={() => setShowSubMenu(!showSubMenu)}>x </button>
                          </div>}
                          {showSubMenu && (subMenu?.map((label, to, iconName,) => {
                            <li
                              className={`px-3 py-2 rounded-sm mb-0.5 last:mb-0 ${pathname === to && "bg-slate-900"
                                }`}
                            >
                              <NavLink
                                end
                                to={to}
                                className={`block text-slate-200 hover:text-white truncate transition duration-150 ${pathname === to && "hover:text-slate-200"
                                  }`}
                              >
                                <div className="flex items-center">
                                  <SVGIcon name={iconName} />
